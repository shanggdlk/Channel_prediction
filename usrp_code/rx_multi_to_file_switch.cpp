...
int UHD_SAFE_MAIN(int argc, char *argv[]) {
    ...
    do {
        ...
        if (type == "double") recv_to_file<std::complex<double> >(rx_usrp, "fc64", file_name, spb, settling, total_num_samps);
            ...
            stop_signal_called = true;
            std::system("sshpass -p \"elshaddoll\" ssh -o StrictHostKeyChecking=no zijian@128.237.153.155 \"echo elshaddoll | sudo -S killall -9 tx_samples_from_file_switch\"");
        }
        std::system("sshpass -p \"elshaddoll\" ssh -o StrictHostKeyChecking=no zijian@128.237.153.155 \"echo elshaddoll | sudo -S killall -2 tx_samples_from_file_switch\"");
        if (rx_freq == end_freq) {
            stop_signal_called = true;
        }
        else {
